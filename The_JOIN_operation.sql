...
SELECT stadium, COUNT(matchid) FROM game JOIN goal ON matchid=id
WHERE teamid = team1 OR teamid = team2 GROUP BY stadium
...
SELECT matchid, mdate, COUNT(matchid) FROM goal JOIN game ON matchid = id WHERE teamid='GER' GROUP BY matchid
